...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:guess_incorrect) { described_class.new(6, '3') }
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(guess_incorrect).not_to be_game_over
      end
    end
  end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game) { described_class.new(1, '1', 5) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats = "That was hard. It took you 5 guesses!\n"
        expect { game.final_message }.to output(congrats).to_stdout
      end
    end
  end
end
