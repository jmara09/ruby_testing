...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:hot_choco) { described_class.new('milo') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(hot_choco.type).to eq('milo')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(hot_choco.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:coffee) { described_class.new('espresso', 5) }
      # remove the 'x' before running this test
      it 'is not full' do
